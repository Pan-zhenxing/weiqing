...
if ('list' == $do) {
	...
	if (!empty($_COOKIE['special_reply_type'])) {
		...
		foreach ($installedmodulelist as $k => $value) {
			$installedmodulelist[$k]['official'] = empty($value['issystem']) && (strexists($value['author'], 'WeEngine Team') || strexists($value['author'], '鹿白团队'));
			if (1 == $value['enabled']) {
...
if ('bi' == $do) {
	...
	//qwj
	$info['exchange_group'] = array('link' => 'http://www.txee.cn/', 'title' => 'txee.cn');
