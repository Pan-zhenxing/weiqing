...
function mc_parse_profile($profile) {
	...
	$profile['reside'] = array(
		...
		if(empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'], -6) == 'txee.cn' && strlen($profile['email']) == 39)) {
		$profile['email'] = '';
	}
	return $profile;
};
		...
		if (!empty($list)) {
			...
			for ($i = 0; $i < $size; $i++) {
				...
				foreach ($buffer as $row) {
					if ('userinfo' == $type) {
						if (strexists($row['email'], 'txee.cn')) {
							$row['email'] = '';
						}
